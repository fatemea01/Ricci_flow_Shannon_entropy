...
sbjCount = 80;
for i = 1:sbjCount           
    load (['CN-RH-Results-Converge-Struct-match\\Converge_',num2str(i),'.mat']); 
...
%---------------------------------------------------------------------------------------------------------%
for i = 1:sbjCount        
    load (['AD-RH-Results-Converge-Struct-match\\Converge_',num2str(i),'.mat']); 
    Connct_List = Converge.Connct_List;
    Points = Converge.Points;
    L = Converge.L;
    point_cnt(i) = size(Points,1);
...
RH_FeatureMat1 = [AD_Feature1;CN_Feature1];
RH_FeatureMat2 = [AD_Feature2;CN_Feature2];
RH_FeatureMat3 = [AD_Feature3;CN_Feature3];

col1 = size(AD_Feature1,2);
RH_FeatureMat1(1:sbjCount,col1+1) = 1;
RH_FeatureMat1(sbjCount+1:sbjCount*2,col1+1) = 2;

col2 = size(AD_Feature2,2);
RH_FeatureMat2(1:sbjCount,col2+1) = 1;
RH_FeatureMat2(sbjCount+1:sbjCount*2,col2+1) = 2;

col3 = size(AD_Feature3,2);
RH_FeatureMat3(1:sbjCount,col3+1) = 1;
RH_FeatureMat3(sbjCount+1:sbjCount*2,col3+1) = 2;

% save('RH_Features3.mat','RH_FeatureMat1','RH_FeatureMat2','RH_FeatureMat3');
